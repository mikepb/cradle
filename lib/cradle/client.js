...
var Client = exports.Client = function(options) {
    options.connector = new Connector(options);

    if (options.servers) {
      for (var i = 0, len = options.servers.length, server; i < len; ++i) {
        server = options.servers[i];
        if (!server.connector) {
          server.connector = new Connector(server);
        }
      }
    }
...
Client.prototype._rawRequest = function (promise, method, path, data,
    ...
    var request = options.connector.request({
        path: path,
        method: method,
        headers: cradle.merge({}, options.headers, headers)
    });
